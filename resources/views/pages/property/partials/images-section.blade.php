...
<section x-data="propertyImagesManager({
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">อัปโหลดได้ไม่จำกัด สามารถกด Ctrl + คลิก เพื่อเลือกหลายไฟล์พร้อมกัน</p>
        </div>
    ...
    <div class="rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-4 py-8 text-center transition" :class="dragOver ? 'border-brand-300 bg-brand-50/50' : ''" @dragover.prevent="dragOver = true" @dragleave.prevent="dragOver = false" @drop.prevent="dropFiles($event)">
                ...
                <template x-for="image in images" :key="image.id">
                    <div class="group relative overflow-hidden rounded-xl border border-gray-200 bg-white shadow-theme-xs">
                        ...
                        <div class="absolute inset-x-0 top-0 flex items-start justify-between gap-1 p-2">
                            <span x-show="image.isDefault" class="inline-flex rounded-md bg-brand-500 px-2 py-0.5 text-xs font-medium text-white">รูปหลัก</span>
                            ...
                            <div class="ms-auto flex gap-1 opacity-0 transition group-hover:opacity-100">
                                <button type="button" x-show="!image.isDefault" @click="setDefault(image)" :disabled="loadingId === image.id" class="rounded-md bg-white/90 p-1.5 text-gray-700 shadow-theme-xs transition hover:bg-white disabled:opacity-50" title="ตั้งเป็นรูปหลัก">
                                    <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                    </svg>
                                </button>
                                <button type="button" @click="remove(image)" :disabled="loadingId === image.id" class="rounded-md bg-white/90 p-1.5 text-error-500 shadow-theme-xs transition hover:bg-white disabled:opacity-50" title="ลบรูป">
                                    <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
