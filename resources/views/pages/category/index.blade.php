        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white">
            ...
            @if ($data->isEmpty())
                <div class="px-5 py-16 text-center sm:px-6">
                    <p class="text-base font-medium text-gray-800">ยังไม่มีประเภทบทความ</p>
                    <p class="mt-1 text-sm text-gray-500">เริ่มต้นด้วยการเพิ่มประเภทบทความใหม่</p>
                </div>
            @else
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-y border-gray-100">
                                ...
                                <th class="px-4 py-3 text-start font-normal">
                                    <span class="text-theme-sm text-gray-500">สถานะ</span>
                                </th>
                                <th class="px-4 py-3 text-start font-normal">
                            ...
                            @foreach ($data as $item)
                                <tr class="hover:bg-gray-50/60">
                                    <td class="px-5 py-4 sm:px-6">
                                        <span class="text-theme-sm font-medium text-gray-800">{{ $item->seq ?? '-' }}</span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="text-theme-sm font-medium text-gray-800">{{ $item->name }}</span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="line-clamp-2 text-theme-sm text-gray-600">{{ $item->decription ?: '-' }}</span>
                                    </td>
                                    <td class="px-4 py-4">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="text-theme-sm text-gray-700">
                                            <p>บทความ {{ number_format($item->articles_count) }}</p>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @if ($item->articles_count > 0)
                                                <span
                                                    ...
                                                    title="ไม่สามารถลบได้ เนื่องจากมีบทความที่ใช้งานอยู่"
                                                >
                                                    ลบ
                                                </span>
                                            @else
                                                <form
                                                    ...
                                                    onsubmit="return confirm('ยืนยันการลบประเภทบทความนี้?')"
                                                >
                                                    @csrf
                                                    @method('DELETE')
                                                    <button
