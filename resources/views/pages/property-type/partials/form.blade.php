...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="px-5 py-6 sm:px-6">
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div>
            <label for="code" class="mb-1.5 block text-theme-sm font-medium text-gray-700">รหัส <span class="text-error-500">*</span></label>
            <input id="code" type="text" name="code" value="{{ old('code', $item->code) }}" required maxlength="45" pattern="[A-Za-z0-9_-]+" title="ภาษาอังกฤษหรือตัวเลขเท่านั้น ห้ามมีช่องว่าง" placeholder="เช่น house" class="h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 @error('code') border-error-500 @enderror" />
            <p class="mt-1.5 text-theme-xs text-gray-500">ภาษาอังกฤษหรือตัวเลขเท่านั้น ห้ามมีช่องว่าง (อนุญาต - และ _)</p>
            @error('code')
            <p class="mt-1 text-theme-xs text-error-500">{{ $message }}</p>
            @enderror
        </div>
