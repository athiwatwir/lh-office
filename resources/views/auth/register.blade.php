...
@section('auth-title', __('Sign Up'))
@section('auth-subtitle', __('Enter your details to create an account!'))
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <x-auth.input id="email" type="email" name="email" :value="old('email')" required
                    autocomplete="username" placeholder="info@gmail.com" />
            ...
            <div>
                ...
                <div x-data="{ showPassword: false }" class="relative">
                    <input id="password" name="password" x-bind:type="showPassword ? 'text' : 'password'" required
                        ...
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pr-11 pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:text-white/90" />
...
@section('auth-footer')
    <p class="text-center text-sm font-normal text-gray-700 sm:text-start dark:text-gray-300">
        ...
        <a href="{{ route('login') }}" class="text-brand-500 hover:text-brand-600">{{ __('Sign In') }}</a>
    </p>
@endsection
