document.addEventListener('alpine:init', () => {
    Alpine.data('userSortable', (config) => ({
        ...
        rows() {
            return [...this.$refs.tbody.querySelectorAll('tr[data-user-id]')];
        },
        ...
        onDragStart(event) {
            ...
            this.dragId = row.dataset.userId;
            this.dragRow = row;
        ...
        onDragOver(event) {
            ...
            if (!targetRow || targetRow.dataset.userId === this.dragId) {
                return;
            }
        ...
        async saveOrder() {
            ...
            const order = this.rows().map((row) => row.dataset.userId);
